...
class SocketInfoWidget(QtGui.QWidget):
    ...
    def setModel(self, model):
        ...
        self.lock_socket.connect(model.set_locked)
    ...
    def update(self):
        ...
        item_selected = 0
        iter = 0
        for fuse_id in self.model.get_model().get_all_fuses():
            iter += 1
            fuse_id_cb.addItem(fuse_id)
            if fuse_id == self.model.get_fuse_id():
                item_selected = iter
    ...
    def apply(self):
        fuse_cb_text = self.findChild(QtGui.QComboBox, 'roomComboBox').currentText()
